...
# Checker APIs (dual — round-robin; owner: /apiconfig, /apiurl, /apienable, /apiworkers)
DEFAULT_CHECKER_APIS = [
    {'id': 'api1', 'name': 'Primary', 'url': 'https://one50con.onrender.com/shopify?', 'max_workers': 150, 'enabled': True},
    {'id': 'api2', 'name': 'Secondary', 'url': 'https://vps-fix-wobh.onrender.com/shopify?', 'max_workers': 150, 'enabled': True},
]
...
PROXY_CHECK_API_URL = 'https://all-three-chi.vercel.app/proxy'
...
def build_chk_range_prompt(cards_count, sites, origin_msg_id, limit_note=None):
    c_1_5, c_1_10, c_all = compute_site_range_counts(sites)
    buttons = [
        [
            Button.inline(f"$1 - $5 ({c_1_5})", f"chk_range:1_5:{origin_msg_id}"),
            Button.inline(f"$1 - $10 ({c_1_10})", f"chk_range:1_10:{origin_msg_id}"),
        ],
        [Button.inline(f"All Sites ({c_all})", f"chk_range:ALL:{origin_msg_id}")],
    ]
    prompt = (
...
_SESSION_PATH = os.path.join(_DATA_DIR, 'evelyn_soul_bot')
...
FORCE_JOIN_CHAT_ID = -1002131846553           # hardcoded channel id
FORCE_JOIN_INVITE = "https://t.me/flex_fr"
...
def _stop_chk_session(user_id, message_id=None):
    ...
    _purge_user_queue(user_id)
    return True
# Track users with an active /st check (one card at a time per user)
active_st_users = set()

# ===== Dynamic worker pool (autoscaling) =====
# Global shared pool used by /chk and /sh. Scales between MIN_WORKERS and
# MAX_WORKERS based on backlog so the bot doesn't overload itself when many
# users check at once.
MIN_WORKERS = 30
MAX_WORKERS = 1000            # baseline soft cap; should be >= sum of enabled API workers
PER_USER_MIN_WORKERS = 20      # GUARANTEED floor of workers per active user
ABSOLUTE_MAX_WORKERS = 1500    # hard safety ceiling so the box can't melt
SCALE_UP_RATIO = 3         # qsize per worker that triggers scale-up (lower = faster ramp)
SCALE_DOWN_RATIO = 1       # qsize per worker that allows scale-down
SCALE_STEP_UP = 25
SCALE_STEP_DOWN = 5
AUTOSCALE_INTERVAL = 1.5   # seconds between autoscale checks (faster reaction)
COOLDOWN_TICKS = 4         # consecutive idle ticks before shrinking (slower cooldown)

# Per-user job queues for round-robin fair scheduling.
# Instead of one global FIFO line (where a 5000-card user would block a
# 10-card user behind them), each user gets their own mini-queue and
# workers rotate through users one job at a time. Effect:
#   - 1 user alone  -> all workers pull from their queue (full speed)
#   - N users       -> each user gets ~workers/N effective throughput
#   - user finishes -> dropped from rotation, remaining users
#                      automatically pick up the freed workers
_user_queues: dict = {}            # user_id -> deque of job dicts
_user_order: "deque" = deque()     # round-robin order of user_ids with work
_user_in_order: set = set()        # O(1) membership for submit_job
_dispatch_job_lock = asyncio.Lock()
_dispatch_event: "asyncio.Event" = asyncio.Event()
_pool_workers: dict = {}   # worker_id -> {'task': Task, 'should_exit': bool}
_pool_started = False
...
def get_dispatch_soft_cap():
    """Dispatch pool target — always at least the sum of enabled API worker limits."""
    mgr = get_checker_manager()
    api_total = mgr.total_workers()
    cfg = load_config()
    manual = cfg.get('dispatch_max_workers')
    if manual is not None:
        try:
            return max(MIN_WORKERS, min(ABSOLUTE_MAX_WORKERS, int(manual)))
        except (TypeError, ValueError):
            pass
    if api_total > 0:
        return max(MIN_WORKERS, min(ABSOLUTE_MAX_WORKERS, api_total))
    return min(MAX_WORKERS, ABSOLUTE_MAX_WORKERS)


def sync_dispatch_pool_to_apis():
    """Grow dispatch pool to match configured API concurrency (no shrink here)."""
    target = get_dispatch_soft_cap()
    current = len(_pool_workers)
    if current >= target:
        return target
    add = min(target - current, ABSOLUTE_MAX_WORKERS - current)
    for _ in range(add):
        _spawn_worker()
    return target
...
def _total_pending() -> int:
    return sum(len(q) for q in _user_queues.values())


def _purge_user_queue(user_id):
    """Drop queued (not yet running) jobs for a user — fixes stuck /chk after stop."""
    q = _user_queues.pop(user_id, None)
    if not q:
        return
    try:
        _user_order.remove(user_id)
    except ValueError:
        pass
    _user_in_order.discard(user_id)
    while q:
        job = q.popleft()
        fut = job.get('future')
...
async def _next_job():
    """Round-robin pick: rotate to next user with pending work and pop one job."""
    while True:
        async with _dispatch_job_lock:
            while _user_order and not _user_queues.get(_user_order[0]):
                uid = _user_order.popleft()
                if not _user_queues.get(uid):
                    _user_queues.pop(uid, None)
                _user_in_order.discard(uid)
            if _user_order:
                uid = _user_order[0]
                _user_order.rotate(-1)
                job = _user_queues[uid].popleft()
                return job
        _dispatch_event.clear()
        await _dispatch_event.wait()

async def _pool_worker(worker_id: int):
    state = _pool_workers.get(worker_id)
    if state is None:
        return
    while not state.get('should_exit'):
        try:
            job = await asyncio.wait_for(_next_job(), timeout=1.0)
        except asyncio.TimeoutError:
            continue
        try:
            await job['run']()
            ...
            try:
                ...
                pass
    _pool_workers.pop(worker_id, None)

def _spawn_worker():
    global _worker_id_seq
    _worker_id_seq += 1
    wid = _worker_id_seq
    _pool_workers[wid] = {'should_exit': False}
    _pool_workers[wid]['task'] = asyncio.create_task(_pool_worker(wid))
...
async def _autoscaler():
    ...
    while True:
        ...
        try:
            current = len(_pool_workers)
            qsize = _total_pending()

            active_users = max(
                sum(1 for q in _user_queues.values() if q),
                len(active_chk_users),
            )

            dynamic_floor = max(
                MIN_WORKERS,
                active_users * PER_USER_MIN_WORKERS,
            )
            dynamic_floor = min(dynamic_floor, ABSOLUTE_MAX_WORKERS)

            soft_cap = max(get_dispatch_soft_cap(), dynamic_floor)
            soft_cap = min(soft_cap, ABSOLUTE_MAX_WORKERS)

            ratio = qsize / max(current, 1)

            if current < dynamic_floor:
                add = min(max(SCALE_STEP_UP, dynamic_floor - current),
                          ABSOLUTE_MAX_WORKERS - current)
                for _ in range(add):
                    _spawn_worker()
                _idle_ticks = 0
            elif ratio > SCALE_UP_RATIO and current < soft_cap:
                add = min(SCALE_STEP_UP, soft_cap - current)
                for _ in range(add):
                    _spawn_worker()
                _idle_ticks = 0
            elif ratio < SCALE_DOWN_RATIO and current > dynamic_floor:
                _idle_ticks += 1
                if _idle_ticks >= COOLDOWN_TICKS:
                    remove = min(SCALE_STEP_DOWN, current - dynamic_floor)
                    marked = 0
                    for wid, st in list(_pool_workers.items()):
                        if marked >= remove:
                            break
                        if not st.get('should_exit'):
                            st['should_exit'] = True
                            marked += 1
                    _idle_ticks = 0
            else:
...
async def ensure_pool_started():
    ...
    async with _pool_lock:
        ...
        for _ in range(MIN_WORKERS):
            _spawn_worker()
        asyncio.create_task(_autoscaler())
        _pool_started = True

async def submit_job(user_id, coro_factory):
    """Enqueue a no-arg async callable onto the given user's queue.
    Workers pull from users in round-robin so users get a fair share of the
    pool. Returns an asyncio.Future that resolves with the coroutine's result."""
    await ensure_pool_started()
    # Use the running loop explicitly — get_event_loop() is deprecated in
    # 3.10+ when called from inside a running coroutine.
    ...
    q.append({'run': _run, 'future': fut})
    if user_id not in _user_in_order:
        _user_order.append(user_id)
        _user_in_order.add(user_id)
    _dispatch_event.set()
    return fut
# ===== End dynamic worker pool =====
...
def format_bin_lookup_ui(bin_number: str, data: dict) -> str:
    ...
    quote_body = tg_html_blockquote(
        f'🎯 <b>BIN:</b> <code>{bin_safe}</code>',
        f'🏛️ <b>Bank:</b> {bank}',
        f'💎 <b>Brand:</b> {brand}',
        f'🎯 <b>Type:</b> {type_line}',
        f'🌍 <b>Country:</b> {country_line}',
    )
    return (
        f'<b>⚡ 💎 BIN Lookup 💎 ⚡</b>\n'
        f'<b>━━━━━━━━━━━━━━━━━</b>\n'
...
def _pick_chk_site(sites, used_sites, bad_sites):
    ...
    if not available:
        available = [s for s in sites if s not in used_sites] or list(sites)
...
async def send_final_results(target_chat_id, results, owner_user_id=None):
    ...
    async with aiofiles.open(filename, 'w', encoding='utf-8') as f:
        ...
        await f.write("Format: CC | Gateway | Price | Message | #SiteNum | MaskedSite\n")
        await f.write("=" * 70 + "\n\n")

        await f.write(f"💎 CHARGED ({len(results['charged'])}):\n")
        await f.write("-" * 70 + "\n")
        for r in results['charged']:
            site_display = mask_site(r.get('site', ''), current_sites)
            await f.write(f"{r['card']} | {r.get('gateway', 'Unknown')} | {r.get('price', '-')} | {r['message'][:100]} | {site_display}\n")
        await f.write("\n")
    ...
    try:
        await safe_send(
            lambda: bot.send_message(target_chat_id, premium_emoji(summary), file=filename, parse_mode='html'),
            user_id=target_chat_id,
        )
    finally:
    ...
    if sub == 'add' or event.is_reply:
        ...
        if sub == 'add':
            if ' ' in text.split('\n')[0]:
                ...
                proxies_to_add.extend(inline + newline)
            else:
                proxies_to_add.extend([ln.strip() for ln in text.split('\n')[1:] if ln.strip()])
        ...
        valid = [p for p in proxies_to_add if _is_valid_proxy_format(p)]
        invalid = len(proxies_to_add) - len(valid)
        if not valid:
...
async def bin_lookup_command(event):
    ...
    try:
        ...
        if not data:
            ...
            return
        await event.reply(
            format_bin_lookup_ui(bin_number, data),
            parse_mode='html',
        )
    except Exception as e:
        await event.reply(
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if ' ' in text.split('\n')[0]:
                ...
                proxies_to_add = inline_proxies + newline_proxies
            else:
        ...
        current_proxies = load_user_proxies(user_id)
        # Reject malformed entries up-front so junk like 'first' never reaches the checker
        valid_input = [p for p in proxies_to_add if _is_valid_proxy_format(p)]
        invalid_count = len(proxies_to_add) - len(valid_input)
        new_proxies = [p for p in valid_input if p not in current_proxies]
        dupes = len(valid_input) - len(new_proxies)
...
async def _check_command_impl(event):
    ...
    prompt, buttons = build_chk_range_prompt(total_cards, global_sites, status_msg.id, limit_note=limit_note)
    await safe_edit(status_msg, prompt, force=True, user_id=target_chat_id, buttons=buttons)
    return


@bot.on(events.CallbackQuery(pattern=b"chk_range:(.*):(.*)"))
async def chk_range_callback(event):
    ...
    user_id = event.sender_id
    pending = PENDING_CHK.pop(origin_msg_id, None)
...
async def _run_chk_session(event, user_id, target_chat_id, username, cards, chk_sites, status_msg, range_label=None, user_entity=None):
    ...
    try:
        ...
        async def run_card(card):
            ...
            res = await check_card_with_retry(
        ...
        # Enqueue every card onto the shared autoscaling pool
        futures = []
        for card in cards:
            fut = await submit_job(user_id, lambda c=card: run_card(c))
            futures.append(fut)
...
async def apiconfig_handler(event):
    ...
    for api in apis:
        ...
        text += premium_emoji(
            f'\n\n<b><code>{api["id"]}</code></b> ({api.get("name", api["id"])}) — {state}\n'
            f'Workers: {api.get("max_workers", 15)}\n'
    ...
    if api_key == 'all':
        ...
        await event.reply(
            premium_emoji(
                ...
                f'🚀 Dispatch pool target: <b>{get_dispatch_soft_cap()}</b> '
                f'(active: <b>{len(_pool_workers)}</b>)'
            ),
            parse_mode='html',
        )
        return
...
async def dispatchworkers_handler(event):
    """Optional manual dispatch pool cap (defaults to sum of API workers)."""
    if not is_owner(event.sender_id):
        return
    parts = (event.text or '').split()
    if len(parts) != 2:
        await event.reply(
            premium_emoji(
                f'Usage: <code>/dispatchworkers 300</code> (1–{ABSOLUTE_MAX_WORKERS})\n'
                'Use <code>/dispatchworkers auto</code> to follow API worker total.'
            ),
            parse_mode='html',
        )
        return
    ...
    if parts[1].lower() == 'auto':
        cfg.pop('dispatch_max_workers', None)
        save_config(cfg)
        await ensure_pool_started()
        sync_dispatch_pool_to_apis()
        await event.reply(
            premium_emoji(
                f'✅ Dispatch pool follows API workers.\n'
                f'Target: <b>{get_dispatch_soft_cap()}</b> (active: <b>{len(_pool_workers)}</b>)'
            ),
            parse_mode='html',
        )
        return
    try:
        ...
        await event.reply(premium_emoji(f'Must be a number 1–{ABSOLUTE_MAX_WORKERS} or <code>auto</code>.'), parse_mode='html')
        return
    n = max(MIN_WORKERS, min(ABSOLUTE_MAX_WORKERS, n))
    cfg['dispatch_max_workers'] = n
    save_config(cfg)
    await ensure_pool_started()
    sync_dispatch_pool_to_apis()
    await event.reply(
        premium_emoji(
            f'✅ Dispatch pool cap set to <b>{n}</b> (active: <b>{len(_pool_workers)}</b>)'
        ),
        parse_mode='html',
    )
...
async def reloadapis_handler(event):
    ...
    await event.reply(
        premium_emoji(
            ...
            f'🚀 Dispatch target: <b>{get_dispatch_soft_cap()}</b> (active: <b>{len(_pool_workers)}</b>)'
        ),
        parse_mode='html',
    )


@bot.on(events.NewMessage(pattern=r'^/workers$'))
async def workers_handler(event):
    if not is_owner(event.sender_id):
        return
    mgr = get_checker_manager()
    pool_workers = len(_pool_workers)
